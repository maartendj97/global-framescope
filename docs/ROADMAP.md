...
- [x] **Duplicate-event clustering** — done: `clusterArticles` (`src/lib/external/gnews.ts`) groups same-story headlines within a category by shared significant keywords (overlap-coefficient ratio ≥0.5, min 2 shared keywords) before mapping to events, so a story covered by multiple publishers becomes one event carrying a `sources: {publisher, url}[]` list instead of one card per publisher. Wired into the "N sources" badge on Home/Events cards via `getEventSourceCount` (`src/lib/eventDisplay.ts`), which also fixes those badges always reading 0 for live events (they only ever matched the mock `Source` records by id). Heuristic, not proven against real live duplicate cases yet — revisit the threshold if it over- or under-merges in practice.
- [ ] **Surface "not covered by" as its own signal** — the framing AI already notices and states when a country's press doesn't cover a story at all (observed live 2026-07-19: RU/CN/IR coverage was silent on the US betting-scandal story, and the model said so in prose). That's a distinctive insight — no other news app shows *silence* as data — but today it's buried in a paragraph. Surface it as its own UI element (e.g. "Not covered by: 🇷🇺 🇨🇳 🇮🇷"). Cheap — the signal already exists in the AI output, this is a rendering/parsing change.
