...
- [x] **Observability dashboard** — done, code-complete and locally verified 2026-07-20 (not yet deployed — see below): `src/proxy.ts` (App Router's renamed `middleware.ts`) records a page view on every real request via `event.waitUntil()`, hashing IP+day (`VISIT_HASH_SALT`) into a per-day Redis Set (`src/lib/external/visitUsage.ts`) so unique visitors are counted without ever storing a raw IP. `GET /api/admin/metrics` (bearer-token gated by `METRICS_API_TOKEN`, 404s when unset) exposes visits plus the existing GNews/NewsData/Anthropic daily-usage counters as one JSON snapshot. `observability-dashboard/` is a separate Next.js app in this repo (own `package.json`/`tsconfig`/`eslint.config.mjs`, excluded from the main app's lint/typecheck) — one mobile-friendly page of stat tiles, single shared-password login (`DASHBOARD_PASSWORD`), fetches metrics server-side with `cache: "no-store"` so the bearer token never reaches the browser. Meant to deploy as its own Vercel project (Root Directory = `observability-dashboard`) so it keeps working if the main app's build breaks — not yet deployed; still needs a GitHub push and Vercel project import plus the three env vars set by hand (all account-level actions, so left for the user rather than done automatically). Per-country content-quality tiers and per-source error-rate aggregation (deliberately deferred in the original proposal) remain unbuilt.
