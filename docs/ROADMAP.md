...
- [x] **Observability dashboard** — done and **live** as of 2026-07-20: `src/proxy.ts` (App Router's renamed `middleware.ts`) records a page view on every real request via `event.waitUntil()`, hashing IP+day (`VISIT_HASH_SALT`) into a per-day Redis Set (`src/lib/external/visitUsage.ts`) so unique visitors are counted without ever storing a raw IP. `GET /api/admin/metrics` (bearer-token gated by `METRICS_API_TOKEN`, 404s when unset) exposes visits plus the existing GNews/NewsData/Anthropic daily-usage counters as one JSON snapshot — verified live in production with real numbers. The dashboard itself is deployed at [framescope-observability-dashboard.vercel.app](https://framescope-observability-dashboard.vercel.app), in its **own separate GitHub repo/Vercel project** (`maartendj97/framescope-observability-dashboard`) rather than a subfolder of this one — an initial attempt at a monorepo subfolder (Vercel Root Directory) hit a Turbopack/Vercel root-detection conflict (Turbopack pulled this repo's `src/proxy.ts` and `postcss.config.mjs` into the dashboard's build; Vercel's output-tracing step separately mis-computed the `.next` path), and splitting into a fully separate repo resolved it cleanly. One mobile-friendly page of stat tiles, single shared-password login, fetches metrics server-side with `cache: "no-store"` so the bearer token never reaches the browser. Per-country content-quality tiers and per-source error-rate aggregation (deliberately deferred in the original proposal) remain unbuilt.
